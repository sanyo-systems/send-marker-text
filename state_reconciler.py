...
def reconcile_state(handler):
    ...
    # keyセット作成
    queue_keys = {tuple(x["key"]) for x in queue if "key" in x}
    failed_keys = {tuple(x["key"]) for x in failed if "key" in x}

    # ======================================
    # ① historyにあるものは削除
    # ======================================
    queue = [x for x in queue if tuple(x["key"]) not in history]
    failed = [x for x in failed if tuple(x["key"]) not in history]

    # ======================================
    # ② queueとfailedの重複 → failed優先
    # ======================================
    queue = [x for x in queue if tuple(x["key"]) not in failed_keys]
    ...
    for item in queue:
        handler.inflight_keys.add(tuple(item["key"]))
